...
- 🔜 Next planned item: Room-specific cleaning on-device verification — validate the current `clean_rooms_miot()` + `clean-extend-data` sequence against real X20+ hardware, capture the actual post-command state transitions, and tighten the verification/fallback heuristics with device evidence instead of cloud-only inference. (Planned 2026-05-01)
