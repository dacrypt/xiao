...
- 🔜 Next planned item: Room-specific cleaning still needs on-device verification — test whether the current `clean_rooms_miot()` preload of `clean-extend-data` plus `siid 2 aiid 3` actually starts room jobs on real X20+ hardware, and capture any accepted-but-no-op behavior against the community reports. (Planned 2026-04-24)
