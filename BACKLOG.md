...
- 🔜 Next planned item: Mission Control vacuum-extend diagnostics copy cleanup — relabel the already-known `siid 4` raw fields (`mop-mode`, `waterbox-status`, `break-point-restart`, `carpet-press`) in the advanced diagnostics panel instead of leaving them as generic `Unknown` slots. (Planned 2026-05-12)
