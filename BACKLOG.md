...
- 🔜 Next planned item: Room-specific cleaning on-device verification — validate whether the current `clean_rooms_miot()` `clean-extend-data` preload plus `siid 2 aiid 3` action actually moves X20+ hardware, and if not, add a safer user-facing diagnostic/fallback path instead of trusting `code=0` alone. (Planned 2026-04-30)
