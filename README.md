...
| Exit codes | `0` success · `1` generic · `2` not configured · `77` auth failed · `80` room-clean accepted but still docked/charging after verification ([AGENTS.md](AGENTS.md#exit-codes)) |
...
If the X20+ accepts a room-clean command but still looks docked/charging a few seconds later, `xiao clean` now exits with code `80` and warns that Xiaomi can acknowledge room actions without actually moving the robot. In that case, re-check room IDs with `xiao map rooms` and fall back to `xiao start` if needed.
